...
app.get("/api/shopify/redirect", async (req, res) => {
  ...
  // obtain Shopify authorization data after redirect
  let shopify_auth = await redirect(req.query);

  // check if Shopify authorization failed
  ...
  // store the obtained access token and shop information in environment variables for time being
...
app.post(
  "/api/shopify/products/:product_id/images",
  multiParty(),
  async (req, res) => {
    let payload = {
